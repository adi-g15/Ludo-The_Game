...
function App() {
	...
	const move = (current_place) => {
		...
		fetch('https://adig15.herokuapp.com/ludo/move/goti', {
			...
			.then(response => {
				if(response.bools[0]) {
					const updated_place = response.coords[0][0] + '-' + response.coords[0][1];	// updated coordinates from response
	...
	async function checkPossible(roll) {
		...
		return new Promise((resolve, reject) => {
			...
			if(flag) {
				fetch('https://adig15.herokuapp.com/ludo/move/goti', {						// If all unlocked gotis are at the last part
					method: 'post',															// check if move is actually possible according
					headers: {																// to their current positions and die value.
						'Content-Type': 'application/json',									// If yes, ask for the move
					},																		// If no, give turn to next player if die < 6
					body: JSON.stringify({													// If no & die = 6, Unlock a goti if there
						'col': ['G', 'Y', 'B', 'R'][turn],									// are locked gotis; and give a re-chance to
						'coords': coords,													// roll the die.
						'dist': roll
					})
				})
					.then(res =>  res.json())
