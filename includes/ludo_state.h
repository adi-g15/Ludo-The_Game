...
public:
	state_goti(colours, direction, coordinate);
...
class ludo_state{
	...
	std::vector<coordinate> lockedPositions;
	std::map<colours, std::vector<coordinate>> movingColours;	//DONT UPDATE THIS, EVEN IF UPDATED THE BOARD, SINCE IT WILL LATER BE USED TO RESET THE BOARD

	player currPlayer;
	colours currColour;

	void update(const game*);
	state_box& getBox(const coordinate&);
	const state_box& getBox(const coordinate&) const;
