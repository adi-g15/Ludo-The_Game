...
*/
	...
	bool thinkerMove(colours gotiColour);	//Uses 'lots' of midlessMovers to gather moveProfit from each, then pick out the best ones
	bool mindlessMovers ( unsigned short roll, std::vector<unsigned short> dieNumbers, unsigned short gotiIndex, std::vector<std::shared_ptr<ludo_goti>> extraUnlockedGotis, std::pair<std::vector<combination>, int> prevMoves, std::map< std::vector<combination>, int>& completeMoves);
	...
	*/
	...
	inline unsigned short getNumGotis(colours) const;
