...
short game::moveGoti(std::shared_ptr<ludo_goti> the_goti, _smartMoveData moveData){
	...
	if ( (getBoardBox(_ludo_coords.start_coords[currentGotiColour]).inBoxGotis.size() + numfinishedGotis[currentGotiColour] + getNumLockedGotis(currentGotiColour)) > 4 ){ //DEBUG
		cout<<"inBoxGotis -> "<<getBoardBox(_ludo_coords.start_coords[currentGotiColour]).inBoxGotis.size()<<endl
			...
			<<"movingGotis -> ";
		for( auto &i : movingGotis[currentGotiColour] ) cout<<i->curr_coords<<' ';
		cout<<endl;
		DEBUG_PRINTBOARD
	...
	cout<<"[DEBUG] INSIDE MOVE, Started moving from "<<the_goti->curr_coords<<"\tfinalCoords"<<finalCoords<<endl;
	cout<<"movingGotis[curr].size() = "<<movingGotis[currentGotiColour].size()<<endl;
	for( auto &i : movingGotis[currentGotiColour]) cout<<i->curr_coords<<' ';	//DEBUG
	cout<<endl;
	...
	if( getBoardBox(finalCoords).box_type == _boxHOME_END ){
		cout<<"DEBUG HOME START, with references to goti =  "<<the_goti.use_count()<<endl;

		auto& lockBox = getBoardBox( getEmptyLocks(currentGotiColour) );
		//DEBUG - Move above line to after 'Removing form MovingGotis'
			cout<<"[DEBUG] GOT LOCKBOX at "<<lockBox.coords<<endl;
			if(lockBox.coords == make_pair(0,0)){
				...
				DEBUG_NUMERICALBOARD
				cout<<endl;
				throw endApplication("Invalid lockbox");
		}
		...
		if ( (getBoardBox(_ludo_coords.start_coords[currentGotiColour]).inBoxGotis.size() + numfinishedGotis[currentGotiColour] + getNumLockedGotis(currentGotiColour)) > 3 ){
			cout<<"inBoxGotis -> "<<getBoardBox(_ludo_coords.start_coords[currentGotiColour]).inBoxGotis.size()<<endl
				<<"Finished -> "<<numfinishedGotis[currentGotiColour]<<endl
				<<"LockedGotis -> "<<getNumLockedGotis(currentGotiColour)<<endl
				<<"movingGotis -> ";
			...
			DEBUG_PRINTBOARD
			DEBUG_NUMERICALBOARD
			throw endApplication("eXTRA GOTIS punGING");
		}
...
bool game::handleMoveVal(short moveVal, vector<unsigned short>& dieNumbers, bool isRobot){	//! Messages won't print for robots
	try{
...
bool game::autoMove(colours gotiColour){	//! Return values same as the moveGoti_function
	...
	for( auto &currentRoll : dieNumbers ){	//Outer loop iterates through the gotis
		...
		if( currentRoll == 6 ){
			if( random()%2 == 1 && getNumLockedGotis(gotiColour)>0 ){	//50% chance of robot using 6 to unlockGoti
				unlockGoti();
				continue;
			}
		}
		...
		do{	//! ie. gotis still remaining to try
			if( triedGotis_indices.size() == movingGotis[currentGotiColour].size() ){
				cout<<currentRoll<<" can't be utilized for any move"<<endl;
				break;	//Break out to the outer loop, thus rejecting currentRoll
			}

			auto random_goti_index = random()%(movingGotis[currentGotiColour].size()) ;	// Randomly selected a goti to move this much units

			try{
				if( movingGotis[currentGotiColour].at(random_goti_index)->curr_direction == NO_TURN ) cout<<"[DEBUG] GIVING INVALID COORD GOTI TO MOVE\n";
				moveObj = isMovePossible(movingGotis[currentGotiColour].at(random_goti_index), currentRoll);
				if( moveObj.isPossible ){
					wasSuccess = true;
					auto moveVal = moveGoti(movingGotis[currentGotiColour][random_goti_index], moveObj );
				...
				cerr<<e.what();
				endApplication("AutoMove->Out_Of_Range exception");
			}
			...
			moveObj.isPossible = false;	//Resetting
		}while( !moveObj.isPossible );
...
bool game::thinkerMove(colours gotiColour){
	vector<unsigned short> dieNumbers;
	map<vector<combination>, int> completeMoves;
	...
	unordered_set<unsigned short> usedRolls; 
	...
	for(size_t index=0; index<movingGotis[currentGotiColour].size(); ++index)
        mindlessMovers(0, dieNumbers, index, {}, {{}, 0}, completeMoves);	//The genesis/starter

        //Now we have the completeMoves vector filled
	for( auto& comb : completeMoves ){  //! Chosing the best move
		if( comb.second > maxProfit ){
			maxProfit = comb.second;
			bestCombination = comb.first;
		}
	}

	//Now moving using the bestCombination found ;D
	for( auto& p : bestCombination ){
		...
		auto [ isPossible, smartData ] = isMovePossible(movingGotis[currentGotiColour][p.first], p.second );
		if( !isPossible )    return false;

			//add other conditions here
		auto moveVal = moveGoti(movingGotis[currentGotiColour][p.first], smartData);
		handleMoveVal( moveVal, dieNumbers );
	}
	...
	//unlockedGotis will be used, so that movingGotis doesn't be modified, since that may cause 'gotiIndex', to wrongly point as well as more than 'actual gotis' in movingGotis, when different branches update movingGotis, but dont remove gotis added by them, so as to give original movingGotis to next big branch
bool game::mindlessMovers ( unsigned short roll, vector<unsigned short> dieNumbers, unsigned short gotiIndex, vector<shared_ptr<ludo_goti>> extraUnlockedGotis, pair<vector<combination>, int> prevMoves, map< vector<combination>, int>& completeMoves){
	if( dieNumbers.empty() ){
		//This is the time for that 'heuristic' function's task
		completeMoves.insert(prevMoves);
		return true;
	}

	auto [ isPossible, smartData ] = isMovePossible(movingGotis[currentGotiColour][gotiIndex], roll);
    if( roll ){
    	if( roll == 6 ){
    		if( isPossible ){
    			if( smartData.moveProfit > UNLOCK ){
    				auto box = getBoardBox(smartData.finalCoords);
		            if( (box.box_type == _boxHOME_END) || (box.areOpponentsPresent(currentGotiColour) &&  box.box_type == _boxNORMAL) ){
					    rolldie(dieNumbers);
					}
    			...
    			}else if( getNumLockedGotis(currentGotiColour)>0 ){	//Maybe the move was not possible on the goti above, but if it was 6, and there are locked gotis available, lets UNLOCK
    ...
    unordered_set<unsigned short> set_rolls;

	for(size_t index=0; index < (movingGotis[currentGotiColour].size() + extraUnlockedGotis.size()) ; ++index){
		for( auto r : dieNumbers ){
			if( set_rolls.find(r) != set_rolls.end() )	continue;
			set_rolls.insert(r);
			mindlessMovers(r, dieNumbers, index, extraUnlockedGotis, prevMoves, completeMoves);
        }
    }
...
bool game::unlockGoti(){
	if (currentGotiColour == UnknownColour)
	{
		...
		cout<<"\nGoti to unlock from "<<box_iter->get().coords<<"\nBEFORE "<<getBoardBox(_ludo_coords.start_coords[currentGotiColour]).inBoxGotis.size()<<endl;
		...
		cout<<"AFTER "<<getBoardBox(_ludo_coords.start_coords[currentGotiColour]).inBoxGotis.size()<<endl;
		if ( (getBoardBox(_ludo_coords.start_coords[currentGotiColour]).inBoxGotis.size() + numfinishedGotis[currentGotiColour] + getNumLockedGotis(currentGotiColour)) > 4+1 ){ //DEBUG
			cout<<"inBoxGotis -> "<<getBoardBox(_ludo_coords.start_coords[currentGotiColour]).inBoxGotis.size()<<endl
				<<"Finished -> "<<numfinishedGotis[currentGotiColour]<<endl
				<<"LockedGotis -> "<<getNumLockedGotis(currentGotiColour)<<endl
				<<"movingGotis -> ";
...
unsigned short game::getNumLockedGotis(colours gotiColour){
	auto num = 0;
...
bool game::lockGoti(std::shared_ptr<ludo_goti> goti_to_lock){	//wrong gotiColour goti called to be removed, currentPlayer was different
	...
	cout<<"[DEBUG] LOCK GOTI START and got real_lockBox = "<<emptyBox->get().coords<<endl;	
...
vector<unsigned short> game::rolldie(){
...
void game::rolldie(vector<unsigned short>& Vec){
	...
	Vec.push_back(6); Vec.push_back(1);	return; //Debug purposes
...
unsigned short game::getNumGotis(colours gotiColour) const{
	return movingGotis.find(gotiColour)->second.size();
}

std::pair<int,int> game::getEmptyLocks(colours gotiColour) const{
	for (auto const &lockedBox : lockedPositions.find(gotiColour)->second)
	{
		if(lockedBox.get().isEmpty() && lockedBox.get().box_type == _boxLOCK){
