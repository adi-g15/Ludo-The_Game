...
ludo_box::ludo_box(const coord& coord, BoxType type) : coords(coord), box_type(type){}

bool ludo_box::areOpponentsPresent(Colour colour) const{
	auto colour_char = colourCodes.at(colour);

	for( auto const& ch : content ){
		if( !isdigit(ch) && ch != colour_char && box_type == BoxType::NORMAL ){
			return true;
		}
	}
	return false;
...
bool ludo_box::removeGoti(std::shared_ptr<ludo_goti>& toBeRemoved){
	...
	for( auto& boxGoti : inBoxGotis ){
		...
		if( boxGoti == toBeRemoved ){
			...
			if( isEmpty() && box_type == BoxType::STOP ){
					content = "X";
			}
...
short ludo_box::appendGoti(std::shared_ptr<ludo_goti> goti_to_add){
	Colour gotiColour = goti_to_add->get_goti_colour();
	auto colour_char = colourCodes.at(goti_to_add->gotiColour);

	if( goti_to_add->get_coords() == coord(0, 0) ) return -1;

	if( box_type == BoxType::STOP && isEmpty() )	content = colour_char;
	...
	if( areOpponentsPresent(gotiColour) && box_type == BoxType::NORMAL ) return 0;
...
bool ludo_box::isEmpty() const{
	if( box_type == BoxType::UNUSABLE )	return false;
